...
class MainActivity : AppCompatActivity() {
    ...
    fun tempsDeLecture(chemin: InputStream): String {
        val fis = chemin as FileInputStream
        val bis = BufferedInputStream(fis)
        var temps : Long=0 // on met long car la fct renvoie un gros nombre trop grand qui ne peut pas etre dans un int
        fis.use {
            val debut = System.currentTimeMillis()
            //Cette ligne force le système à lire le fichier du premier jusqu'au dernier octet sans rien stocker, juste pour consommer le flux.
            //Lire le fichier octet par octet jusqu'à la fin (bis.read() != -1)
            while (bis.read() != -1); //; signifie que la boucle while est vide. Elle se contente d'exécuter bis.read() en boucle jusqu'à ce que tout le fichier soit parcouru et qu'elle reçoive -1
